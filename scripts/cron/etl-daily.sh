...
if [ "${ETL_SKIP_AGENT:-}" != "1" ]; then
  ...
  for attempt in $(seq 1 $((DEBATE_MAX_RETRIES + 1))); do
    log "▶ 토론 에이전트 시도 ${attempt}/$((DEBATE_MAX_RETRIES + 1))"
    if npx tsx src/agent/run-debate-agent.ts >> "$LOG_FILE" 2>&1; then
      log "✓ 토론 에이전트 완료"
      debate_success=1
      break
    else
      log "✗ 토론 에이전트 실패 (시도 ${attempt}/$((DEBATE_MAX_RETRIES + 1)))"
      if [ "$attempt" -lt $((DEBATE_MAX_RETRIES + 1)) ]; then
  ...
  if [ "$debate_success" -eq 1 ]; then
    ...
    log "✗ 토론 에이전트 $((DEBATE_MAX_RETRIES + 1))회 시도 모두 실패 — 일간보고서 스킵"
    send_error "토론 에이전트 $((DEBATE_MAX_RETRIES + 1))회 시도 실패 — 수동 재실행 필요" "토론"
