...
# 경로에 공백이 있으면 cron이 파싱 못함
validate_paths() {
  if [[ "$PROJECT_DIR" == *' '* ]] || [[ "$SCRIPT_DIR" == *' '* ]]; then
    echo "오류: 경로에 공백이 포함되어 있습니다. cron이 올바르게 동작하지 않습니다." >&2
...
# Debate Daily: 평일 UTC 22:00 (KST 07:00) — Mon-Fri
0 22 * * 1-5 $SCRIPT_DIR/debate-daily.sh >> $PROJECT_DIR/logs/cron.log 2>&1
# Agent Weekly: 토 UTC 01:00 (KST 10:00)
...
${entries}" | crontab -
  ...
  echo "  Debate Daily: 평일 UTC 22:00 (KST 07:00)"
