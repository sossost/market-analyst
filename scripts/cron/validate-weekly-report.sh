#!/bin/bash
#
# 주간 보고서 품질 검증 — Claude Code CLI 기반
#
# 주간 에이전트(agent-weekly.sh) 완료 후 후속 실행.
# 최신 주간 리포트를 DB에서 조회 → claude -p로 검증 → 점수 미달 시 GitHub 이슈 생성.
#
# Usage:
#   ./scripts/cron/validate-weekly-report.sh
#
# 필수 환경변수:
...
LOG_FILE="$LOG_DIR/validate-weekly-$(date +%Y-%m-%d).log"
QA_DIR="$PROJECT_DIR/data/report-qa"
PROMPT_TEMPLATE="$PROJECT_DIR/scripts/validate-weekly-report-prompt.md"
TIMEOUT_SEC=300
TOTAL_SCORE_MAX=50
TOTAL_SCORE_THRESHOLD=35
TOTAL_SCORE_THRESHOLD_NO_NOVELTY=28
...
# --- Step 1: 최신 주간 리포트 조회 ---
log "▶ 최신 주간 리포트 조회"
REPORT_JSON=$(npx tsx src/scripts/get-latest-weekly-report.ts 2>>"$LOG_FILE")
...
log "✓ 주간 리포트 조회 완료 — 이번 주: $REPORT_DATE, 직전: $PREV_DATE"
...
if [ "$HAS_PREV" = "false" ]; then
  echo "" >> "$PROMPT_FILE"
  echo "참고: 직전 주간 리포트가 없습니다. novelty 항목은 null로 표시하세요." >> "$PROMPT_FILE"
fi
...
QA_RESULT=""
if $TIMEOUT_CMD "$TIMEOUT_SEC" env -u ANTHROPIC_API_KEY claude -p --output-format json < "$PROMPT_FILE" > "$CLAUDE_RAW_FILE" 2>>"$LOG_FILE"; then
...
# --- Step 4: 결과 저장 ---
QA_FILE="$QA_DIR/weekly-$REPORT_DATE.json"
echo "$QA_RESULT" | jq --arg date "$REPORT_DATE" '. + {reportDate: $date, reportType: "weekly", validatedAt: (now | todate)}' > "$QA_FILE"
...
log "점수: $TOTAL_SCORE/$TOTAL_SCORE_MAX | 이슈: $HAS_ISSUE | $SUMMARY"
...
if [ "$HAS_ISSUE" = "true" ]; then
  if [ "${VALIDATE_DRY_RUN:-}" = "1" ]; then
    ...
    ISSUE_TITLE="주간 리포트 품질 이슈 — $REPORT_DATE (점수: $TOTAL_SCORE/$TOTAL_SCORE_MAX)"
    ISSUE_TITLE=$(echo "$ISSUE_TITLE" | head -c 100)

    # 이슈 본문을 파일로 전달 (인수 인젝션 방지)
    echo "$QA_RESULT" | jq -r '.issueBody // ""' > "$ISSUE_BODY_FILE"

    # 우선순위 결정: 3점 미만 항목 있으면 P1, 아니면 P2 (jq로 처리)
    PRIORITY_LABEL="P2: medium"
    if echo "$QA_RESULT" | jq -e '
      [.scores.factConsistency, .scores.bullBias, .scores.structure, .scores.verifiability] +
