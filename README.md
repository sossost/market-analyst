...
QA Gate ── 품질 검증 → 조건부 발송
        ...
        ▼
Delivery
  일간 리포트(Discord) │ 주간 리포트(HTML + Discord) │ 기업 리포트(S등급 종목)
```
...
| 리포트 | 주기 | 채널 | 핵심 내용 |
|--------|------|------|----------|
| 일간 브리핑 | 평일 장 마감 후 | Discord | 시장 온도 + 멀티게이트 + Thesis-Aligned 후보 + 관심종목 현황 |
| 주간 분석 | 토요일 | Discord + HTML | 섹터 로테이션 + 관심종목 궤적(Top 5~7) + 포트폴리오 현황 |
