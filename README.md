...
2. 멀티 모델 애널리스트 토론 (매일 22:00 UTC)
   ...
   → CREDIT 카테고리 뉴스 수집: 신용 스프레드/CLO/하이일드/PE 리스크 → 신용 경색 시그널 (#749)
   → LLM 뉴스 테마 추출: 수집 뉴스 배치 → Claude Sonnet → 인과적 섹터 영향 테마 (severity/sourceCount 필터) → HIGH 테마 토론 컨텍스트 주입 (#751)
   → 뉴스 사각지대 힌트 주입: Gap Analyzer가 식별한 미수집 테마를 토론 컨텍스트에 주입 → 에이전트 자율 검색 동기 부여 (#750)
...
5. 품질 관리 (자동)
   ...
   → 콘텐츠 QA: 나레이션 존재 검증 + 데이터-해석 톤 일관성 + HTML 렌더링 완전성 (#693)
...
7. 에이전트 리포트
   ...
   → 주간: 지표 4×2 그리드(10Y·DXY·공포탐욕 통합) + 시장 브레드스(Phase 분포·5일 추이·LLM 해석) + 섹터 로테이션 + 업종 RS Top 10 + tracked_stocks 궤적(선별 기준: featured→P2 streak→detection_lag, Top 5~7 주목 + 주봉 관점) + 포트폴리오 승격/탈락(LLM 심사 → source='agent' 등록/해제) + Phase 2 구간 분류 + 다음 주 관전 포인트 (HTML 리포트 + Discord 요약)
...
| System Audit | 06:00 토 | 데이터 무결성 + 코드-DB 정합성 + 파이프라인 연결성 + 테스트/빌드 자동 감사 → 이슈 자동 생성 |
