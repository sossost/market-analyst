...
### Step 4: 맥미니 크론탭 업데이트
**에이전트**: 비서실장 직접 실행 (SSH 명령)
**작업**: `ssh mini@100.77.162.69 "cd /Users/mini/market-analyst && ./scripts/cron/setup-cron.sh"`
**완료 기준**: `crontab -l`에 QA Weekly (토 UTC 03:00) 항목이 나타남

### Step 5: 통합 검증
**에이전트**: 비서실장 직접 확인
**작업**: `ssh mini@100.77.162.69 "crontab -l"` 로 QA 항목 확인, workflow YAML 문법 검증
