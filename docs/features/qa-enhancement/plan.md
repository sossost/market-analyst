...
**실현 가능성 판단**: 현재 프롬프트를 완전히 교체하면 기존 QA 이력과 비교 불가해진다. 대신 기존 5항목에 `eventAwareness` 항목을 추가하고 총점을 50점으로 확장하는 방식이 현실적이다.

**2-1. validate-debate-report-prompt.md 항목 추가**
- 기존 4개 항목(thesisBasis/bullBias/analystDiversity/structure) 유지
- 신규 항목 추가: `eventAwareness` (0~10점) — 주요 매크로 이벤트(FOMC, CPI 등) 인지 여부
- 총점: 40점 → 50점 (novelty는 여전히 선택)
- 이슈 기준 업데이트: `totalScore ≤ 32` (기존 40점 기준 28 → 50점 기준 32로 비례 조정)
