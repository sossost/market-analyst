...
- **tech 에이전트 전망 주의**: 기술/산업 전망도 반드시 지수 또는 섹터 RS 기반 정량 조건을 포함하세요. 예: "Technology RS > 65", "NASDAQ > 18000". 정량 조건이 없는 tech thesis는 자동 검증이 불가능하여 ACTIVE 상태로 적체되고, 학습 루프(agent_learnings)에 반영되지 않습니다. 검증 불가 thesis는 진행률 50% 초과 시 강제 만료됩니다.
- **structural_narrative 해소 조건 필수**: structural_narrative thesis는 구조적 방향성을 다루지만, 반드시 정량적 해소 조건(targetCondition, invalidationCondition)을 수치 비교 형식으로 작성하세요. "AI capex 사이클 전환 가속" 같은 정성적 thesis도 "Technology RS >= 60" 또는 "NASDAQ > 18000" 등 수치로 검증 가능한 조건을 포함해야 합니다. 에이전트당 ACTIVE thesis 상한은 7건이므로, 판정 불가 thesis가 쌓이면 새 thesis 생성 여력이 줄어듭니다.
