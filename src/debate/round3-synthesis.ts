...
- 현재 추적 중인 병목의 상태 (ACTIVE / RESOLVING / RESOLVED / OVERSUPPLY)
  ...
  - **[nextBottleneck 작성 규칙 — Tier 구분 필수]**
    - **Tier 1 (tech 에이전트 공급망 연역 추론)**: tech 에이전트가 1명이라도 공급망 구조에서 구체적 노드를 제시하면 기록 허용.
      정량 근거 가드레일 **면제** (연역 추론은 데이터 선행이 목적).
      단, confidence: 'low' 필수, nextBottleneck 문자열 끝에 "(추론 기반, 미확인)" suffix 필수.
    - **Tier 2 (귀납 합의, 2명 이상)**: 2명 이상이 동일 지점을 언급한 경우 기록. 정량 근거(CAPEX 규모, 리드타임 단축 수치, 재고 증가율 등) **1개 이상 필수**. 정량 근거 없으면 null로 기록.
...
**nextBottleneck 작성 규칙 (Tier 구분):**
- structural_narrative 카테고리에만 작성. 그 외는 null.
- **Tier 1 (tech 에이전트 공급망 연역 추론)**: tech 에이전트가 1명이라도 공급망 구조에서 구체적 노드를 제시하면 기록 허용.
  - 정량 근거 가드레일 **면제** (공급망 연역 추론은 데이터 선행이 목적).
  - 필수: confidence: 'low', nextBottleneck 문자열 끝에 "(추론 기반, 미확인)" suffix.
  - 예: "HBM 용량 제한 — GPU 병목 해소 후 2~3분기 내 (추론 기반, 미확인)"
- **Tier 2 (귀납 합의)**: 라운드 1·2에서 2명 이상이 동일 지점을 언급한 경우에만 작성.
  - 정량 근거(CAPEX 규모, 리드타임 단축 수치, 재고 증가율 등) 1개 이상 필수. 정량 근거 없으면 null.
  - confidence는 원본(합의 수준 기반) 그대로 보존.
