...
${sanitized}
  ...
  if (debateWeeklySummary != null && debateWeeklySummary !== "") {
    const sanitized = sanitizeXml(debateWeeklySummary);
    prompt += `

<debate-weekly-summary trust="internal">
${sanitized}
</debate-weekly-summary>

**활용법**:
- 병목 상태 변화(ACTIVE→RESOLVING 등)는 해당 섹터의 향후 전개 판단에 반영
- 주간 주도섹터 합의(5일 중 3일+ 언급)는 sectorRotationNarrative에서 "주간 토론 합의"로 참조
- 반복 과열 경고 종목은 riskFactors에서 언급
- debateInsight 필드에 주간 토론 누적 흐름을 2~3문장으로 서술`;
  }
