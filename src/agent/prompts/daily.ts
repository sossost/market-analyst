...
4개 narrative 필드는 반드시 { "headline": "...", "detail": "..." } 객체로 작성하라. 문자열로 작성 금지.
...
Phase 전환을 서술할 때 아래 톤을 반드시 적용하라. Phase 2(상승 추세)가 최적 상태이며, Phase 2에 가까워지면 긍정, 멀어지면 부정이다.

| 전환 | 의미 | 톤 | 용어 예시 |
|------|------|-----|----------|
| Phase 1→2 | 바닥 → 상승 진입 | 긍정 | "바닥 돌파", "상승 전환" |
| Phase 2→3 | 상승 → 분배 진입 | 주의 | "분배 진입", "과열 경고" |
| Phase 3→4 | 분배 → 하락 전환 | 부정 | "하락 전환", "약세 확인" |
| Phase 4→1 | 하락 → 바닥 구축 시작 | 긍정 | "바닥 구축 시작", "하락 둔화" |
| Phase 3→2 | 분배 → 상승 복귀 | 중립~긍정 | "상승 복귀", "분배 탈출" |
| Phase 2→1 | 상승 → 추세 이탈 | 부정 | "추세 이탈", "상승 실패" |

**금지**: Phase 3→2 전환을 "붕괴", "악화", "약세"로 프레이밍하는 것은 모델 정의와 충돌한다. Phase 3(분배)에서 Phase 2(상승)로 돌아오는 것은 개선 방향이다.`;
...
unusualStocksNarrative 작성 시 관련 종목이 있으면 체인명을 언급하세요.
