...
2. **섹터 RS 랭킹 표 + 섹터별 요약** — 섹터별 RS 점수와 순위 변동. Group Phase와 **4주/8주 RS 변화량(change4w/change8w)**, Phase 2 종목비율을 표에 포함하세요. 이 3가지가 Group Phase 판정 근거이므로, 독자가 "RS는 올랐는데 왜 Phase 악화?"를 스스로 판단할 수 있어야 합니다. 전일 대비 순위 변동이 큰 섹터(±3 이상)는 별도 한 줄 코멘트 추가. **섹터 RS 표 바로 아래에 업종 RS Top 10 테이블을 추가하라** (컬럼: 순위 \| 업종명 \| 소속섹터 \| RS \| 4주 변화 \| Phase \| P2 비율). 데이터는 분석 워크플로우 step 3에서 \`mode="industry"\`로 조회한 결과를 사용하라.
