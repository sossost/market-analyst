...
4. **업종 RS 주간 변화 조회** (get_leading_sectors, mode: "industry")
   - 반환되는 \`changeWeek\` 필드를 기준으로 내림차순 정렬 — 이번 주 RS 변화량이 가장 큰 업종 10개 추출
...
| 순위 | 업종 | 섹터 | RS | 주간 변화 | Phase 2 비율 |
|------|------|------|----|-----------|--------------|
| 1 | Semiconductors | Technology | 72.3 | ▲+8.2 | 57% |
| 2 | ... | ... | ... | ... | ... |
...

해석: [상위 3개 업종의 공통점 또는 자금 흐름 방향 1~2줄. 예: "Technology 섹터 내 하드웨어 중심 자금 유입 — 반도체 + 통신장비 동반 상승으로 섹터 RS 반등 임박 신호"]
\`\`\`
...
9. **개별 종목 심층 분석** (get_stock_detail) — 등록 후보의 상세 데이터 확인
