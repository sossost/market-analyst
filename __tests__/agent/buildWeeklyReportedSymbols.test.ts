...
describe("buildWeeklyReportedSymbols", () => {
  ...
  it("gate5 종목은 5중게이트 reason으로 기록된다", () => {
    const data = makeData({
      gate5Candidates: [
        {
          symbol: "COIN",
    ...
    expect(result[0].reason).toBe("5중게이트");
...
describe("getWeeklySourceCounts", () => {
  ...
  it("각 소스별 건수를 정확히 집계한다", () => {
    const data = makeData({
      ...
      activePortfolioSymbols: ["META", "TSLA"],
    });
...
describe("formatSourceCounts", () => {
  it("소스별 건수를 한 줄 문자열로 포맷한다", () => {
    const result = formatSourceCounts({
      gate5: 3, breakout: 1, vcp: 2, thesisAligned: 1, watchlist: 5, portfolio: 2,
    });

    expect(result).toBe("Gate5 3, 돌파 1, VCP 2, 서사수혜 1, 관심종목 5, 포트폴리오 2");
  });

  it("모든 소스가 0이면 전부 0으로 표시한다", () => {
    const result = formatSourceCounts({
      gate5: 0, breakout: 0, vcp: 0, thesisAligned: 0, watchlist: 0, portfolio: 0,
    });

    expect(result).toBe("Gate5 0, 돌파 0, VCP 0, 서사수혜 0, 관심종목 0, 포트폴리오 0");
  });
});
